...
# 定义类型（"define_name",default_value,"Notation"）
...
def predict_to_onehot(label):
    ...
    label_onehot = tf.one_hot(label, depth=FLAGS.letter_num, on_value=1.0,axis=2)  # axis: The axis to fill (default: -1, a new inner-most axis).
...
def captcharec():
    ...
    with tf.variable_scope("soft_cross"):
        # 交叉熵：entropy = -(y_true * log y_predict)   y_true与y_predict有相同的shape
