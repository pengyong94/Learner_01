...
"""构造数据"""
x = np.linspace(-2, 2, 500, dtype=np.float32).reshape([-1,1])
x_data = np.hstack((x,x))                                     # 构造两个参数
bias = np.random.normal(0.5, 0.5, x.shape).astype(np.float32)
y_true = np.add(
    np.add(
        np.multiply(
            5, np.power(
                x, 2)), np.multiply(
                    4, x)), bias)            # 构造一个f(x)= ax*x+bx+c的二次函数
print(y_true.shape)
"""构造计算图"""
# 构造数据输入的参数；占位符
x_batch = tf.placeholder(tf.float32, [None, 2], name="x_batch")
...
# 添加神经网络
h1 = net_layer(x_batch, 2, h1_num, activate_function=activate_func)   # 激活函数选用
...
with tf.Session() as sess:
    ...
    for i in range(step_num):
        ...
        print(loss_data)
        result = sess.run(y_pred, feed_dict={x_batch: x_data})   # 得到的预测值，与y_true同维度
